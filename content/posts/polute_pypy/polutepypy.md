...
Each achievement contains a  intro/fetishization as to why I picked them. The hardcore speedrunner can ignore them and follow along the instructions but for the trivia nerds out there feel free to check em.
...
{{< details "poetry fanboyism" >}}
Man Poetry is just too nice. It even includes a _tests_ folder, its like it wants you to do good. We will come back to this folder later. And the notion of tools guiding you to be a better programmer.
{{< /details >}}
...
Welcome to Conventional Commits, your new best bud. This cool cat helps you craft thoughtful commits, and it even adds emojis with predefined meanings. Check out https://gitmoji.dev/ and the commit history of this mini-shitty project for a taste. You might roll your eyes at another dev tool for a "non-existing problem", like something trivial like commit messages. But hey, it's about letting the tool guide your thoughts, man.

When you call up the extension for a new commit, it throws options at you. Are we refactoring, adding a feature, tweaking docs, fixing bugs, writing tests, or messing with CI? Then, pick an emoji for a single action (https://gitmoji.dev/). Only after that, you write your commit title and maybe a longer description, and a section to adress breaking changes or issues that can be closed.

As you iterate on that workflow, you'll notice that it pushes you to break down your changes into commitable chunks, atomizing your actions in the project. Forcing you to think in isolated bits. The options laid out in front of you not only prompt introspection about the changes you've made but also remind you of potential tasks you haven't tackled yet, like writing tests or setting up CI. Maybe I'm hyping it up for my own narrative, but it's still an awesome tool that helps reducing what can otherwise be a over-complex changes into trackable checkpoints, with the side effect of feeling like you're using modern hieroglyphs.
...
Your package is garbage, useless trash, but that doesn't mean it can't look nice. Here's what you need:
    ...
    Above all, nice looking logo. Have you noticed the very obvious wave of emoji filled docs? Thank you hugging face. They've spread like wildfite cause they're nice to look at. They give live to what otherwise is a bunch of boring text, and documentation. They make tetx easier to digest, and when used correctly they signal or prepare the user to what's about to be read. Aesthetics are to underrated. Why am I talking about emojis when I mention a logo. Well I fucking like logos on things thats why.


# last step

Ideally you would commit the changes as youre making them, often you end up macking changes on the same files for different reasons. But honestly while I'm coding, I often have a goal in mind and then I retroactoively add the commits as atomically as I can? Reemeber that's okay to break dogmas dont go silly on the atomizing. The last step involves commiting all the changes we've made so far. Let's go trhough it. First let's commit the added dependencies.
